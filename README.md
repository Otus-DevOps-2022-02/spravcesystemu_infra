...
* Создаем ветку ansible-2
* Создаем плейбук reddit_app.yml заполняем его и тестируем
* Создаем плейбук на несколько сценариев reddit_app2.yml
* Разбиваем наш плейбук на несколько: app.yml, db.yml, deploy.yml и переименовываем наши старые плейбуки
* Модифицируем наши провижионеры в packer, меняеем их на ansible и перезапекаем образы, указываем новые образы в переменных терраформа.
